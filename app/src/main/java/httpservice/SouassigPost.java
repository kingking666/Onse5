...
public class SouassigPost
{
    // 请求服务器端的url
    private static String PATH = "http://123.207.237.45/webtt/SouassigAction";
    ...
    public static String executeHttpPost(String assigT,String assigS,String page ) {
  ...
  /* public static void main(String[] args)
    {
        SouassigPost po=new SouassigPost();
        displayAssig  field=new displayAssig ();
        Iterator it=new Gsonanalyze().getword(po.executeHttpPost("外卖","已完成","1"));
        Gson gson = new Gson();
        while (it.hasNext()) {
            JsonElement ee = (JsonElement) it.next();
            field = gson.fromJson(ee,displayAssig .class);
            //表示服务器端返回的结果
        }

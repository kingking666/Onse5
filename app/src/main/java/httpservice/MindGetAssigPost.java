...
public class MindGetAssigPost {

    // 请求服务器端的url
    private static String PATH = "http://123.207.237.45/webtt/MindGetAssig";
    private static URL url;

    public MindGetAssigPost() {
    ...
    {
        MindGetAssigPost po=new  MindGetAssigPost();
        t_assig field=new t_assig();
        Iterator it=new Gsonanalyze().getwordSystem.out.print(po.executeHttpPost("18316513128"));
        Gson gson = new Gson();
        while (it.hasNext()) {
            JsonElement ee = (JsonElement) it.next();
            field = gson.fromJson(ee, t_assig.class);
            //表示服务器端返回的结果

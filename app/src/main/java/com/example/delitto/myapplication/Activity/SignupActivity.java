...
public class SignupActivity extends AppCompatActivity {
    ...
    */
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //初始化验证API
        CIAService.init(this, "a732352b3199422e86742fcce7da595a", "d374ff07d9794c2f9116fc2c69da4034");
...
/*
         ...
         Thread signupThread = new Thread(new SignupHandler());
            ...
            */
            new Thread() {
                @Override
                public void run() {
                    final String info = SignupPost.executeHttpPost(_phoneText.getText().toString(), _passwordText.getText().toString());
                   ECTools.ECSignin(_phoneText.getText().toString().trim(), _passwordText.getText().toString().trim());
                    signupHandler.postDelayed(
                            new Runnable() {
                                public void run() {
                                    if (localNumber == 1) {
                                        if(info.equals("signup is success!!!!"))
                                        {
                                            onSignupSuccess();

                                            progressDialog.dismiss();
                                            if (ECTools.refreshSignin()) {
                                                ECTools.ECLogin(_phoneText.getText().toString().trim(), _passwordText.getText().toString().trim());
    ...
    public void onSignupSuccess() {
        ...
        UserThis.userPhone=_phoneText.getText().toString();
        PreferenceUtils.setPrefString(SignupActivity.this,"Ones5User",_phoneText.getText().toString());
        //由于未知原因无法跳转至LoginAcitivy回传信息
        //先用Intent 跳转至MainActivity
       // Intent intent = new Intent(getApplicationContext(), RegistinfoActivity.class);
        startActivity(AppTools.GotoActivity(SignupActivity.this,RegistinfoActivity.class));
